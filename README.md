...
def export*to_json(channel=None, output_file=None):
"""Exporte les messages en format JSON"""
if not output_file:
timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
channel_suffix = f"*{channel.replace('#', '')}" if channel else ""
output*file = f"messages{channel_suffix}*{timestamp}.json"
...
def do_GET(self):
if self.path == '/metrics': # Récupérer les métriques depuis le serveur socket
try:
